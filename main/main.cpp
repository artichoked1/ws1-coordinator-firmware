...
extern "C" void app_main(void) {
  ...
  if (coldBoot || slave_count == 0) {
    ...
    while (sensorbus_hal_get_time_ms() - start < DISCOVERY_WINDOW_MS) { // Start discovery window
  ...
  for (size_t si = 0; si < slave_count; si++) {
    ...
    size_t outn = 0;
    if (sensorbus_pb_decode_sensors(resp.payload, resp.payload_len, out, &outn) !=
        SENSORBUS_OK) {
  ...
  for (size_t si = 0; si < slave_count; si++) {
    ...
    for (size_t i = 0; i < slaves[si].sensor_count; i++) {
      const auto &s = slaves[si].sensors[i];
  ...
  for (size_t si = 0; si < slave_count; si++) {
    for (size_t i = 0; i < slaves[si].sensor_count; i++) {
      const auto &sensor = slaves[si].sensors[i];
      // sensor.format is your 3-bit fmt code
      sensorbus_pb_add_sensor(&lb, sensor.type, sensor.format, sensor.index,
                    sensor.value);
    }
  }
  ...
  if (uplink_len > 0) {
    ...
    ESP_LOGI(TAG, "sendReceive → %d, FCntUp now %" PRIu32, radioLibState,
             node.getFCntUp());
    memcpy(LWsession, node.getBufferSession(),
